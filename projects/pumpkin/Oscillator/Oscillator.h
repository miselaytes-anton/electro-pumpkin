...
class Oscillator {
	public:
		Oscillator(){};
		Oscillator(float frequency, float fs = 41000, unsigned int type = sine, float initialPhase = 0)
		{
			setup(frequency, fs, type, initialPhase);
		}
		~Oscillator(){};
		
		void setup(float frequency, float fs = 41000, unsigned int type = sine, float initialPhase = 0)
		{
			fs_ = fs;
